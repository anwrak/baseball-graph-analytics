...
class BasicPitching:
    ...
    def calc_cera(df):
        ...
        return np.where(B > 0, np.round(9 * (A/B) - .56, 3), np.nan)
    ...
    def calc_pbabip(df):
        A = df['H'] - df['HR']
        B = BasicPitching.calc_pab(df) - df['K'] - df['HR']
        return np.where(B > 0, np.round(A / B, 3), np.nan)
    ...
    @staticmethod
    def calc_obpa(df):
        return np.where(BasicPitching.calc_pab(df) > 0, np.round((df['H'] + df['BB'] + df['HBP']) / (BasicPitching.calc_pab(df) + df['BB'] + df['HBP'] + df['SF']), 3), np.nan)
